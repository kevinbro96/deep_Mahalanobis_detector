...
parser.add_argument('--outf', default='./data/adv_white/', help='folder to output results')
...
def main():
    ...
    save_model = torch.load('./data/emb2048/model_epoch172.pth')
    ...
    detector = torch.load('./data/2class/0029.ckpt.pth')
    detector.cuda()
    ...
    for data, target in test_loader:
        ...
        output = model(inputs)
        output2 = detector(inputs - vae(inputs))
        loss = criterion(output, target) + criterion2(output2.squeeze(dim=1), torch.ones(target.shape).cuda())
        loss.backward()
